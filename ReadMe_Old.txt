================================================================================
    MFC (MICROSOFT FOUNDATION CLASS) ライブラリ : MFC_RhythmGame プロジェクトの概要
===============================================================================

この MFC_RhythmGame アプリケーションは、アプリケーション ウィザードによって作成されました。このアプリケーションは MFC の基本的な使い方を示すだけでなく、アプリケーションを作成するための開始点でもあります。

このファイルには、MFC_RhythmGame アプリケーションを構成する各ファイルの内容の概要が含まれています。

MFC_RhythmGame.vcxproj
...
MFC_RhythmGame.vcxproj.filters
...
MFC_RhythmGame.h
    ...
    その他のプロジェクト固有のヘッダー (Resource.h など) が含まれ、CMFC_RhythmGameApp アプリケーション クラスを宣言します。

MFC_RhythmGame.cpp
    これはメインのアプリケーション ソース ファイルで、CMFC_RhythmGameApp アプリケーション クラスが含まれています。
...
res\MFC_RhythmGame.ico
    これはアプリケーションのアイコンとして使用されるアイコン ファイルです。このアイコンは、メイン リソース ファイル MFC_RhythmGame.rc にインクルードされます。

res\MFC_RhythmGame.rc2
...
MFC_RhythmGameDlg.h, MFC_RhythmGameDlg.cpp - ダイアログ
    これらのファイルには、CMFC_RhythmGameDlg クラスが含まれます。このクラスはアプリケーションのメイン ダイアログの動作を定義します。ダイアログのテンプレートは、Microsoft Visual C++ で編集できる MFC_RhythmGame.rc に含まれています。
...
StdAfx.h, StdAfx.cpp
    これらのファイルは、MFC_RhythmGame.pch という名前のプリコンパイル済みヘッダー (PCH) ファイルと、StdAfx.obj という名前のプリコンパイル済みの型ファイルをビルドするために使用されます。

Resource.h
    これは、新しいリソース ID を定義する標準のヘッダー ファイルです。このファイルの読み込みおよび更新は、Microsoft Visual C++ で行います。

MFC_RhythmGame.manifest
